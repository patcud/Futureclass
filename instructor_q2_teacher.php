<?php
    ...
    $sql2 = "SELECT * FROM teachers_good WHERE coursesID=$coursesID AND teachersID='$id' and qtype = 'suggestion' AND status=0 ORDER BY likeCount DESC";
        ...
        <div class="row flex-nowrap w-100" style="margin-top: 5rem !important;">
            ...
            <div class="col-11 col-xs-11 col-md-10 bg-light" style="margin-top: 20px;">
                <ul class="nav nav-tabs">
                    <li class="nav-item">
                            <a class="nav-link text-danger" aria-current="page" href="assessment_instructor_teacher.php?id=<?php echo $coursesID; ?>">คะแนน</a>
                        </li>
                    <li class="nav-item">
                        <a class="nav-link text-danger" href="instructor_q1_teacher.php?id=<?php echo $coursesID; ?>">ข้อดี</a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link active" href="instructor_q2_teacher.php?id=<?php echo $coursesID; ?>">ข้อเสนอแนะ</a>
                    </li>
                </ul>
                <!--card แสดงข้อมูลรายละเอียดรายวิชาและการประเมิน ฝั่งด้านซ้าย-->
                <div class="mt-2">
                    <h3>ข้อเสนอแนะต่อผู้สอน</h3>
                    <p style="color:#6e6e6e">[ เป็นการประเมินความเห็น นิสิตสามารถเพิ่มความเห็นได้ตลอดทั้งเทอม และสามารถกด like ความคิดเห็นที่ตรงกับผู้อื่นได้ ]</p>
                </div>
                <hr>
                ...
                <div id='requestment_card' class="row row-cols-1 g-1 bg-light"></div>
                    <?php 
                        while($a = mysqli_fetch_array($r)) {
                            ...
                            $oldreport = mysqli_fetch_array(mysqli_query($con,"SELECT report FROM teachers_good_report WHERE ID=$qid and userEmail = '$_SESSION[email]'"));
                            if (isset($oldreport)) {
                                $report_color = '#ffc74d';
                            }
                            echo '
                            ...
                            if ($report_color == '#979797') {
                                ...
                                <!-- ใส่ php -->
                                <a href="qt_report.php?id='.$qid.'&cid='.$coursesID.'&qn=q2" class="btn btn-danger" role="button">Report</a>
                                </div>
                                </div>
                                </div>
                                </div>'
