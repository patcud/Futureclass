<?php
    ...
    if (isset($_POST["name"]) && ($_POST["name"] != "")) {
        $name=$_POST["name"];
        $sql1 = mysqli_query($con,"UPDATE teachers SET fname='$name' WHERE teachersID = '$id'");   
        $rowsub = mysqli_fetch_array(mysqli_query($con, "SELECT * FROM teachers WHERE teachersID = '$id'"));
        $_SESSION['user'] = $rowsub['fname'] . " " . $rowsub['lname']; 
    }
    
    if (isset($_POST["surname"]) && ($_POST["surname"] != "")) {
        $surname=$_POST["surname"];
        $sql2 = mysqli_query($con,"UPDATE teachers SET lname='$surname' WHERE teachersID = '$id'");
        $rowsub = mysqli_fetch_array(mysqli_query($con, "SELECT * FROM teachers WHERE teachersID = '$id'"));
        $_SESSION['user'] = $rowsub['fname'] . " " . $rowsub['lname']; 
    }
    ...
    $querysub = "SELECT * FROM teachers WHERE teachersID = '$id'";
    ...
    if (isset($_SESSION['error'])) {
        header('location: profile_edit_teacher.php');
    } else {
