<?php
    ...
    $query = "  SELECT *
                FROM courses 
                WHERE subjectsID = '$sid' AND semesterID = $semID AND password = '$password'";
    ...
    if (mysqli_num_rows($result) == 1) {
        ...
        if ($checkStu == 0) {
            ...
            if (!isset($check) || $check[0] != 1) {
                ...
                $addnoti = mysqli_query($con, "REPLACE notifications SET coursesID='$cid', userID='$id', name= '', type= 'assess', requestmentID= '0', message= '', status= 'unread', date= CURRENT_TIMESTAMP");
                /* NEW!!! $addnoti2 */
                $addnoti2 = mysqli_query($con, "REPLACE notifications SET coursesID='$cid', userID='$id', name= '', type= 'tassess', requestmentID= '0', message= '', status= 'unread', date= CURRENT_TIMESTAMP");
            }
        }
